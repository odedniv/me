...
For these situations mocking and isolation in tests work best -- "given _some_ `predicate`, how would `filter` behave?".

Product-specific code is mostly _not_ like that.
Components explicitly care about each others' _exact_ behavior, not "_some_" behavior.
The product itself depends on _exact_ components, not "_some_" components.
...
and it normally relies on its _exact_ parts to function, not "_some_" parts.
...
* **You maintain redundant code** by re-implementing components for tests.
...
* **Debunk:** This might be true, but not based on your arbitrary separation of classes and functions.
  Most of your components likely describe free business logic.
* **Strategy:** For components that _are_ expensive, make them configurable for testing instead of replacing them entirely
  (implementing a DB interface? support an in-memory version to help dependants verify with your important business logic).

> "The other component is difficult to set up, and its edge cases are hard to reproduce"

* **Debunk:** Each components' tests need this setup for their own direct integrations anyway, so this was already done at least once.
* **Strategy:** Create test helpers for your components to make it trivial to test their dependants without replacing them with mocks
...
* **Strategy:** Optimize your build system, not individual tests (e.g. make sure unchanged dependencies are properly cached).
...
If `HumanLeg` depends on `HumanBody` that depends on `HumanBrain`, and `HumanBrain` isn't testable -- don't mock `HumanBody` too.

```python
# DON'T do this:
def test_human_leg():
  fake_human_body = FakeHumanBody() # HumanBrain isn't testable :-(
  human_leg = HumanLeg(fake_human_body)
  ...

# DO this:
def test_human_leg():
  fake_human_brain = FakeHumanBrain() # HumanBrain isn't testable :-(
  human_body = HumanBody(fake_human_brain) # But HumanBody is!
  human_leg = HumanLeg(human_body)
  ...
```
...
Well, not really -- when designing your tests, you should be thinking about _behaviors_, not _components_.
...
* **Human leg test group:** Verifies behaviors of the leg (attached to a body), down to the smallest detail that can be expected from a human leg.
  ...
  For example, this includes moving individual toes.
...
Sometimes a component is so simple that it's not worth distinguishing its test group from its sole dependant.
Distinguishing a component's test group is not required for a minimalistic test design,
but it can sometimes help you find your tests (e.g. `HumanLeg` → `HumenLegTests`),
and avoid repetition -- especially if there are multiple components depending on it.
...
* Every test verifies a behavior of some scoped unit, even if the unit has a lot behind it ("a lot" is subjective too - is a _CPU_ not a lot?).
...
* **It makes you test with undue isolation** to fit the "unit" slot.
* **It makes you test the same behavior multiple times** to "cover" all test types.
* **It makes it confusing where future tests should be** because there is no single source of truth for where behaviors are tested
  (does _this_ behavior need an "integration test" or just a "unit test"?).
...
```python
# DO this:
class HumanBodyTests: <mostly larger tests requiring files and threads>
